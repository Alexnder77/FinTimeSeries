...
arima_Dinterest <- arima(DanishInterestReate$InterestRate)
summary(arima_Dinterest)
fit_arima_Dinterest <- arima(DanishInterestReate$InterestRate, order = c(0, 1, 0))
summary(fit_arima_Dinterest)

#danish unemployment
arima_Dunemployment <- auto.arima(DanishUnemployment$`Procent af arbejdsstyrken`)
summary(arima_Dunemployment)
fit_arima_Dunemployment <- arima(DanishUnemployment$`Procent af arbejdsstyrken`, order = c(0, 1, 1))
...
arima_Sinterest <- auto.arima(SWEInterestReate$Interest_Reate)
summary(arima_Sinterest)
fit_arima_Sinterest <- arima(SWEInterestReate$Interest_Reate, order = c(2, 1, 3))
...
DanishBNP <- DanishBNP %>%
  mutate(
    Date = as.Date(as.yearqtr(YearQtr, format = "%Y Q%q")),
    BNP_Diff = c(NA, diff(BNP)) # Differenced GDP
  )

DanishInflation <- DanishInflation %>%
  mutate(
    Date = as.Date(paste0(year, "-01-01")),
    Inflation_Diff = c(NA, diff(Inflation)) # Differenced Inflation
  )

DanishInterestReate <- DanishInterestReate %>%
  mutate(
    Date = as.Date(paste0(year, "-01-01")),
    InterestRate_Diff2 = c(NA, NA, diff(diff(InterestRate))) # Differenced Interest Rate
  )

DanishUnemployment <- DanishUnemployment %>%
  mutate(
    Date = as.Date(paste0(Year, "-01-01")),
    Unemployment_Diff = c(NA, diff(`Procent af arbejdsstyrken`)) # Differenced Unemployment Rate
  )
...
SWEInflation <- SWEInflation %>%
  mutate(
    Date = as.Date(paste0(Year, "-01-01")),
    Inflation_Diff = c(NA, diff(Inflation)) # Differenced Inflation
  )

SWEInterestReate <- SWEInterestReate %>%
  mutate(
    Date = as.Date(paste0(Year, "-01-01")),
    InterestRate_Diff = c(NA, diff(Interest_Reate)) # Differenced Interest Rate
  )

SWEUnemployment <- SWEUnemployment %>%
  mutate(
    Date = as.Date(paste0(Year, "-01-01")),
    Unemployment_Diff3 = c(NA, NA, NA, diff(diff(diff(Unemployment_Rate)))) # Differenced Unemployment Rate
  )

# Convert OMXC20 and OMXS30 Date columns
OMXC20 <- OMXC20 %>%
...
```{r}
# Merge Danish datasets
DanishData <- full_join(DanishBNP, DanishInflation, by = "Date") %>%
  full_join(DanishInterestReate, by = "Date") %>%
  full_join(DanishUnemployment, by = "Date")

# Merge Swedish datasets
SwedenData <- full_join(SWEBNP, SWEInflation, by = "Date") %>%
  full_join(SWEInterestReate, by = "Date") %>%
  full_join(SWEUnemployment, by = "Date")
```
...
p1_swe <- ggplot(SWEBNP, aes(x = Date, y = BNP)) +
  geom_line(color = "blue") +
  labs(title = "Sweden GDP", x = "Year", y = "GDP") +
  custom_theme

p2_swe <- ggplot(SWEInflation, aes(x = Date, y = Inflation)) +
  geom_line(color = "red") +
  labs(title = "Sweden Inflation", x = "Year", y = "Inflation Rate (%)") +
  custom_theme

p3_swe <- ggplot(SWEUnemployment, aes(x = Date, y = Unemployment_Rate)) +
  geom_line(color = "green") +
  labs(title = "Sweden Unemployment", x = "Year", y = "Unemployment Rate (%)") +
  custom_theme

p4_swe <- ggplot(SWEInterestReate, aes(x = Date, y = Interest_Reate)) +
  geom_line(color = "purple") +
  labs(title = "Sweden Interest Rate", x = "Year", y = "Interest Rate (%)") +
  custom_theme

# Danish Variables Plot
p1_dan <- ggplot(DanishBNP, aes(x = Date, y = BNP)) +
  geom_line(color = "blue") +
  labs(title = "GDP", x = "Year", y = "GDP") +
  custom_theme

p2_dan <- ggplot(DanishInflation, aes(x = Date, y = Inflation)) +
  geom_line(color = "red") +
  labs(title = "Inflation", x = "Year", y = "Inflation Rate (%)") +
  custom_theme

p3_dan <- ggplot(DanishUnemployment, aes(x = Date, y = DanishUnemployment$`Procent af arbejdsstyrken`)) +
  geom_line(color = "green") +
  labs(title = "Unemployment", x = "Year", y = "Unemployment Rate (%)") +
  custom_theme

p4_dan <- ggplot(DanishInterestReate, aes(x = Date, y = InterestRate)) +
  geom_line(color = "purple") +
...
p1_swe <- ggplot(SWEBNP, aes(x = Date, y = BNP_Diff)) +
  geom_line(color = "blue") +
  labs(
    title = "Sweden GDP (First Difference)",
    x = "Year",
    y = "Differenced GDP"
  ) +
  custom_theme

p2_swe <- ggplot(SWEInflation, aes(x = Date, y = Inflation_Diff)) +
  geom_line(color = "red") +
  labs(
    title = "Sweden Inflation (First Difference)",
    x = "Year",
    y = "Differenced Inflation Rate (%)"
  ) +
  custom_theme

p3_swe <- ggplot(SWEUnemployment, aes(x = Date, y = Unemployment_Diff3)) +
  geom_line(color = "green") +
  labs(
    title = "Sweden Unemployment Rate (Third Difference)",
    x = "Year",
    y = "Third Differenced Unemployment Rate (%)"
  ) +
  custom_theme

p4_swe <- ggplot(SWEInterestReate, aes(x = Date, y = InterestRate_Diff)) +
  geom_line(color = "purple") +
  labs(
    title = "Sweden Interest Rate (First Difference)",
    x = "Year",
...
p1_dan <- ggplot(DanishBNP, aes(x = Date, y = BNP_Diff)) +
  geom_line(color = "blue") +
  labs(
    title = "Denmark GDP (First Difference)",
    x = "Year",
    y = "Differenced GDP"
  ) +
  custom_theme

p2_dan <- ggplot(DanishInflation, aes(x = Date, y = Inflation_Diff)) +
  geom_line(color = "red") +
  labs(
    title = "Denmark Inflation (First Difference)",
    x = "Year",
    y = "Differenced Inflation Rate (%)"
  ) +
  custom_theme

p3_dan <- ggplot(DanishUnemployment, aes(x = Date, y = Unemployment_Diff)) +
  geom_line(color = "green") +
  labs(
    title = "Denmark Unemployment Rate (First Difference)",
    x = "Year",
    y = "Differenced Unemployment Rate (%)"
  ) +
  custom_theme

p4_dan <- ggplot(DanishInterestReate, aes(x = Date, y = InterestRate_Diff2)) +
  geom_line(color = "purple") +
  labs(
    title = "Denmark Interest Rate (Second Difference)",
    x = "Year",
